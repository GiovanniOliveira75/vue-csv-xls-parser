<template>
  <div class="catalog-column-chooser">
    ...
    <div>
      <div class="row">
        <div class="col-md-4 mb-4" v-for="(column, index) in localUserColumns">
          <div class="card bg-success mb-3" :class="{ 'bg-success': column.selection, 'bg-danger': !column.selection }">
            <div class="card-header">
              <h6 v-if="column.selection">{{ column.selection.name }}</h6>
              <h6 v-else>{{ text[lang].chooseColumn }}</h6>
            </div>
            <div class="card-body">
              <multiselect
                :id="index"
